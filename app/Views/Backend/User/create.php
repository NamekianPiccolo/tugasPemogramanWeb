...
<div class="mb-8">
    <h1 class="text-3xl font-bold text-white tracking-wide">Tambah User Baru</h1>
    <p class="text-gray-300 mt-2">Isi formulir berikut untuk menambahkan user baru ke sistem.</p>
</div>
    ...
    <form action="<?= base_url('admin/user/store') ?>" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-2">
                ...
                <input type="text" name="username" value="<?= old('username') ?>" class="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-white outline-none focus:ring-2 focus:ring-brand-500 transition-all" required>
            </div>
            
            <div class="space-y-2">
                <label class="text-gray-300 text-sm font-semibold uppercase tracking-wider">Nama Lengkap</label>
                <input type="text" name="nama_lengkap" value="<?= old('nama_lengkap') ?>" class="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-white outline-none focus:ring-2 focus:ring-brand-500 transition-all" required>
            </div>
 
            <div class="space-y-2">
                <label class="text-gray-300 text-sm font-semibold uppercase tracking-wider">Email</label>
                <input type="email" name="email" value="<?= old('email') ?>" class="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-white outline-none focus:ring-2 focus:ring-brand-500 transition-all" required>
            </div>
            ...
            <div class="space-y-2">
                ...
                <select name="role" class="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-white outline-none focus:ring-2 focus:ring-brand-500 transition-all appearance-none">
                    <option value="karyawan" <?= old('role') === 'karyawan' ? 'selected' : '' ?> class="bg-gray-800">Karyawan</option>
                    <option value="admin" <?= old('role') === 'admin' ? 'selected' : '' ?> class="bg-gray-800">Admin</option>
                </select>
            </div>
 
            <div class="space-y-2 md:col-span-2">
                <label class="text-gray-300 text-sm font-semibold uppercase tracking-wider">Password</label>
                <input type="password" name="password" class="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-white outline-none focus:ring-2 focus:ring-brand-500 transition-all" required>
            </div>
        </div>
