...
<div class="mb-8">
    <h1 class="text-3xl font-bold text-white tracking-wide">Edit Kategori</h1>
    <p class="text-gray-300 mt-2">Ubah informasi kategori dokumen yang sudah ada.</p>
</div>
...
<div class="max-w-2xl bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl shadow-2xl p-8">
    <form action="<?= base_url('admin/kategori/update/' . $kategori['id']) ?>" method="post">
        <div class="mb-6">
            ...
            <input type="text" name="nama_kategori" value="<?= esc($kategori['nama_kategori']) ?>" class="w-full bg-[#0f172a]/50 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent transition-all" required>
        </div>
