...
<div class="mb-7 page-intro flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
    <div>
        ...
        <p class="text-sm mt-2" style="color:var(--muted); font-family:'Lora',serif;">Kelola kategori pengelompokkan berkas agar arsip Anda terorganisir dengan rapi.</p>
    </div>
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-7 stats-row">
    ...
    <div class="glass-card p-5 flex items-center justify-between stat-card" style="background-color: #d8e2dc;">
        <div>
            ...
            <p class="text-xs mt-1 flex items-center" style="color:var(--muted); font-family:'Lora',serif;">
    ...
    <div class="glass-card p-5 flex items-center justify-between stat-card" style="background-color: #f2e8cf;">
        <div>
            ...
            <p class="text-xs leading-snug" style="color:var(--muted); font-family:'Lora',serif;">Folder digital berbasis nama unik untuk akses cepat & pencarian.</p>
        </div>
        <div class="w-12 h-12 flex items-center justify-center shrink-0 organic-shape"
    ...
    <div class="glass-card p-5 flex items-center justify-between stat-card" style="background-color: #fae1dd;">
        <div>
            ...
            <p class="text-xs leading-snug" style="color:var(--muted); font-family:'Lora',serif;">Setiap berkas wajib termasuk dalam satu kategori yang telah ditetapkan.</p>
        </div>
        <div class="w-12 h-12 flex items-center justify-center shrink-0 organic-shape"
             style="background:rgba(224,122,95,0.15);border:2px solid var(--txt);box-shadow: 2px 2px 0px var(--txt);">
            <svg class="w-6 h-6" fill="none" stroke="var(--txt)" viewBox="0 0 24 24">
...
<div class="glass-card p-14 text-center" style="background-color: #f2e8cf;">
    ...
    <p class="text-sm" style="color:var(--muted); font-family:'Lora',serif;">Buat kategori pertama untuk mulai mengorganisir dokumen.</p>
</div>
<?php else: ?>
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 kat-grid">
    <?php foreach ($kategori as $k): ?>
    <div class="glass-card kat-card flex flex-col justify-between" style="background-color: #fffcf2;">
        ...
        <div class="p-5 pb-3">
            ...
            <h4 class="text-lg font-bold mb-1 truncate font-kalam" style="color:var(--txt);" title="<?= esc($k['nama_kategori']) ?>">
                <?= esc($k['nama_kategori']) ?>
            </h4>
            <p class="text-xs" style="color:var(--muted); font-family:'Lora',serif;">ID: #<?= esc($k['id']) ?></p>
        </div>

        <!-- Actions -->
        <div class="px-5 pb-6 pt-4 flex space-x-2"
