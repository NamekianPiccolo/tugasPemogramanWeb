...
<div class="mb-7 page-intro flex justify-between items-center">
    <div>
        ...
        <h1 class="text-xl font-bold grad-cyan mb-0.5">Distribusi & Peminjaman Berkas</h1>
        <p class="text-xs" style="color:var(--muted);">Pantau dan kelola riwayat peminjaman dokumen fisik atau serah terima berkas antar divisi.</p>
    </div>
    <a href="<?= base_url("$role/distribusi/create") ?>"
       class="btn-violet px-4 py-2.5 rounded-xl text-xs font-bold flex items-center space-x-2 shrink-0 cursor-pointer">
    ...
    <?php if (empty($distribusi)): ?>
        <div class="glass-card p-14 text-center" style="background-color: #f2e8cf;">
            <div class="w-16 h-16 mx-auto mb-4 flex items-center justify-center organic-shape"
                 style="background:rgba(6,182,212,.15);border:2px solid var(--txt);box-shadow: 2px 2px 0px var(--txt);">
            ...
            <p class="text-sm" style="color:var(--muted); font-family:'Lora',serif;">Belum ada riwayat peminjaman dokumen terdaftar.</p>
        </div>
    <?php else: ?>
        <?php 
        ...
        <div class="glass-card flex flex-col md:flex-row md:items-center justify-between p-5 anim-item organic-shape" 
            ...
            <div class="flex-1 min-w-0 flex items-start md:items-center gap-5">
                ...
                <div class="w-12 h-12 flex items-center justify-center shrink-0 organic-shape"
                     style="background:rgba(6,182,212,0.15); border:2px solid var(--txt); box-shadow: 2px 2px 0px var(--txt);">
                    <span class="font-kalam font-bold text-lg" style="color:var(--txt);"><?= $i ?></span>
                </div>
                
                <!-- Details -->
                <div class="flex-1 min-w-0">
                    <h4 class="text-lg font-bold font-kalam truncate mb-1" style="color:var(--txt);" title="<?= esc($d['peminjam']) ?>">
                        <?= esc($d['peminjam']) ?>
                    </h4>
                    <p class="text-sm font-bold truncate mb-2" style="color:var(--muted); font-family:'Lora',serif;" title="<?= esc($d['judul'] ?? '') ?>">
                    ...
                    <div class="flex flex-wrap items-center gap-3 text-[10px] font-bold uppercase tracking-widest font-kalam" style="color:var(--dim);">
                        <span class="organic-shape px-2.5 py-1" style="background:rgba(61, 64, 91, 0.08); border:1px solid rgba(61, 64, 91, 0.2);">
                            Pinjam: <?= esc(date('d M Y', strtotime($d['tanggal_pinjam']))) ?>
                        </span>
                        <span class="organic-shape px-2.5 py-1" style="background:rgba(61, 64, 91, 0.08); border:1px solid rgba(61, 64, 91, 0.2);">
                            Kembali: <?= $d['tanggal_kembali'] ? esc(date('d M Y', strtotime($d['tanggal_kembali']))) : '—' ?>
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-5 md:mt-0 pt-4 md:pt-0 flex flex-row md:flex-col items-center md:items-end justify-between md:justify-center shrink-0 gap-3"
                 style="border-top:2px dashed rgba(61, 64, 91, 0.15); border-top-width: 2px; md:border-top-width: 0;">
...
(() => {
    ...
    gsap.fromTo('.list-container', { y: -14, opacity: 0 }, { y: 0, opacity: 1, duration: .5, ease: 'back.out(1.5)', delay: .12 });
    gsap.fromTo('.anim-item', { y: -10, opacity: 0 }, { y: 0, opacity: 1, duration: .35, stagger: .05, ease: 'back.out(1.5)', delay: .2 });
})();
</script>
<?= $this->endSection() ?>
