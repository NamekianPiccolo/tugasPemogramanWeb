...
<?php else: ?>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 doc-grid">
        ...
        foreach ($dokumen as $d):
            ...
            if ($ext === 'pdf') {
                $extColor = 'var(--secondary)'; $extBg = 'rgba(224,122,95,0.15)'; $extBorder = 'var(--secondary)';
            } elseif (in_array($ext, ['doc','docx'])) {
                $extColor = 'var(--txt)'; $extBg = 'rgba(61,64,91,0.15)'; $extBorder = 'var(--txt)';
            } elseif (in_array($ext, ['xls','xlsx','csv'])) {
                $extColor = 'var(--primary)'; $extBg = 'rgba(132,169,140,0.15)'; $extBorder = 'var(--primary)';
            } else {
                $extColor = 'var(--txt)'; $extBg = 'rgba(138,129,124,0.15)'; $extBorder = 'var(--muted)';
            }

            $isLocked = false;
            if ($role === 'karyawan' && ($d['status_izin'] ?? '') !== 'Disetujui') {
                $isLocked = true;
            }
        ?>

        <div class="glass-card group flex flex-col justify-between relative overflow-hidden doc-card" style="background-color: <?= $bgCard ?>;">

            <!-- Lock overlay -->
            <?php if ($isLocked): ?>
            <div style="position:absolute;inset:0;z-index:10;backdrop-filter:blur(3px);
                ...
                <div class="w-12 h-12 mb-3 flex items-center justify-center organic-shape"
                     style="background:rgba(224,122,95,.15);border:2px solid var(--secondary);box-shadow: 2px 2px 0px var(--secondary);">
                    <svg class="w-6 h-6" fill="none" stroke="var(--secondary)" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                              d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/>
                    </svg>
                </div>
                <h5 class="text-base font-bold mb-1 font-kalam" style="color:var(--txt);">Akses Terkunci</h5>
                <p class="text-xs mb-3 max-w-[165px] leading-relaxed" style="color:var(--muted); font-family:'Lora',serif;">
                    Anda memerlukan izin khusus untuk membaca berkas ini.
                </p>
                ...
                <?php elseif (($d['status_izin'] ?? '') === 'Ditolak'): ?>
                    <a href="<?= base_url('karyawan/izin/create') ?>"
                       class="px-4 py-2 organic-shape text-sm font-bold font-kalam text-white transition-all"
                       style="background:var(--secondary); border:2px solid var(--txt); box-shadow: 2px 2px 0px var(--txt);">
                        Ajukan Lagi
                    </a>
                <?php else: ?>
                    <form action="<?= base_url('karyawan/izin/store') ?>" method="POST" class="w-full max-w-[155px]">
                        <?= csrf_field() ?>
                        <input type="hidden" name="dokumen_id" value="<?= $d['id'] ?>">
                        <input type="hidden" name="pesan" value="Meminta izin akses untuk membaca berkas.">
                        <button type="submit" class="w-full px-4 py-2 organic-shape text-sm font-bold font-kalam transition-all cursor-pointer"
            ...
            <div class="p-6 pb-2">
                ...
                <p class="text-[11px] mb-3 truncate font-bold uppercase tracking-widest font-kalam" style="color:var(--secondary);">
                    <?= esc($d['nama_kategori'] ?? 'Kategori Dihapus') ?> / <?= esc($d['nama_unit'] ?? 'Bagian Dihapus') ?>
                </p>
            ...
            <!-- Footer Actions -->
            <div class="px-6 pb-8 pt-5 flex items-center justify-between"
                ...
                <?php if (!$isLocked && !empty($d['file_dokumen'])): ?>
...
(() => {
    ...
    gsap.fromTo(['.alert-box', '.doc-card'], { y: -14, opacity: 0 }, { y: 0, opacity: 1, duration: .45, stagger: .04, ease: 'back.out(1.5)', delay: .1 });
})();
</script>
<?= $this->endSection() ?>
