...
<style>
    ...
    .fl-input:focus {
        ...
        background: #fffcf2;
        box-shadow: 4px 4px 0px rgba(61, 64, 91, .15)
    }
    ...
    input[type="date"].fl-input:focus~.fl-label {
        color: var(--primary)
    }

    /* Upload zone */
    .upload-zone {
        border: 1.5px dashed rgba(61, 64, 91, .2);
        border-radius: 12px;
        padding: 24px 20px;
        text-align: center;
        cursor: pointer;
    ...
    .upload-zone.drag-over {
        ...
        border-style: solid;
        background: rgba(132, 169, 140, .05)
    }
    ...
    .upload-ring {
        ...
        margin: 0 auto 10px;
        border-radius: 50%;
        background: rgba(132, 169, 140, .1);
        border: 1.5px dashed rgba(132, 169, 140, .35);
        display: flex;
        align-items: center;
    ...
    .upload-zone.drag-over .upload-ring {
        border-color: var(--primary);
        border-style: solid;
        background: rgba(132, 169, 140, .15);
    ...
    .file-pill {
        ...
        border-radius: 100px;
        background: rgba(132, 169, 140, .1);
        border: 1.5px solid rgba(132, 169, 140, .35);
        font-size: 11px;
        font-family: 'Kalam', cursive;
        font-weight: 700;
        color: var(--txt)
    ...
    .current-file {
        ...
        padding: 8px 12px;
        border-radius: 9px;
        background: rgba(132, 169, 140, .08);
        border: 1.5px solid rgba(132, 169, 140, .25);
        margin-bottom: 10px;
        font-size: 11px;
        font-family: 'Kalam', cursive;
        font-weight: 700;
...
<form action="<?= base_url("$role/dokumen/update/" . $dokumen['id']) ?>" method="POST" enctype="multipart/form-data">
    ...
    <div class="grid gap-5" style="grid-template-columns:1fr;align-items:start">
        ...
        <div class="glass-card static-card" style="padding:28px 32px;border-radius:14px">
            <div class="sec-sep">
                <div class="sec-sep-line"></div><span>Informasi Dokumen</span>
            </div>
            ...
            <div class="fl-group is-textarea">
                ...
                <svg class="fl-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M4 6h16M4 10h16M4 14h10" />
                </svg>
            </div>

            <?php if ($role !== 'admin'): ?>
                <div class="fl-group is-textarea mb-4" style="margin-top: 16px;">
                    <textarea name="pesan_revisi" id="pesan_revisi" rows="3" placeholder="Pesan / Catatan Revisi" class="fl-input" required><?= old('pesan_revisi') ?></textarea>
                    <label for="pesan_revisi" class="fl-label">Pesan / Catatan Revisi (Detail Perubahan) <span class="req">*</span></label>
                    <svg class="fl-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                    </svg>
                </div>
        ...
        <div class="flex flex-col gap-4">
            <div class="glass-card static-card p-5" style="border-radius:14px">
                <div class="sec-sep">
                    <div class="sec-sep-line" style="background:var(--primary)"></div><span>Ganti File</span>
                </div>
                ...
                <?php if (!empty($dokumen['file_dokumen'])): ?>
                    <div class="current-file">
                        ...
                        <div>
                            <p class="text-[9px] uppercase tracking-widest" style="color:var(--muted)">File Saat Ini</p>
                            <p class="text-[11px]"><?= esc($dokumen['file_dokumen']) ?></p>
                        </div>
                    </div>
                <?php endif; ?>
...
<script>
    (() => {
        ...
        const zone = document.getElementById('uploadZone'),
            input = document.getElementById('file_dokumen'),
            pill = document.getElementById('filePill'),
            nameEl = document.getElementById('fileName'),
            ring = document.getElementById('uploadRing'),
            icon = document.getElementById('uploadIcon');
        ...
        const loadCurrentFilePreview = () => {
            const isPdf = currentFileName && currentFileName.toLowerCase().endsWith('.pdf');
            const isDocx = currentFileName && currentFileName.toLowerCase().endsWith('.docx');
            const isCsv = currentFileName && currentFileName.toLowerCase().endsWith('.csv');
            ...
            if (isPdf) {
                ...
                previewTitle.textContent = "Pratinjau File Saat Ini (Word)";
                previewContainer.style.display = 'block';
                docxPreviewDiv.style.display = 'block';
                docxPreviewDiv.innerHTML = '<p class="text-xs text-gray-500 font-bold p-4 text-center">Mengunduh pratinjau dokumen dari server...</p>';

                fetch(currentFileUrl)
                    .then(res => res.blob())
                    .then(blob => {
                        if (typeof docx !== 'undefined') {
                            docx.renderAsync(blob, docxPreviewDiv)
                                .catch(err => {
                                    docxPreviewDiv.innerHTML = '<p class="text-xs text-red-500 font-bold p-4 text-center">Gagal merender DOCX: ' + err.message + '</p>';
                                });
                        } else {
                            docxPreviewDiv.innerHTML = '<p class="text-xs text-amber-600 font-bold p-4 text-center">Gagal memuat sistem pratinjau Word dari CDN.</p>';
                        }
                    })
                    .catch(err => {
                        docxPreviewDiv.innerHTML = '<p class="text-xs text-red-500 font-bold p-4 text-center">Gagal mengambil file: ' + err.message + '</p>';
                    });
            } else if (isCsv) {
                previewTitle.textContent = "Pratinjau File Saat Ini (CSV)";
                previewContainer.style.display = 'block';
                docxPreviewDiv.style.display = 'block';
                ...
                fetch(currentFileUrl)
                    ...
                    .catch(err => {
                        docxPreviewDiv.innerHTML = '<p class="text-xs text-red-500 font-bold p-4 text-center">Gagal memuat CSV: ' + err.message + '</p>';
                    });
            }
        };
        ...
        const setFile = file => {
            ...
            if (isPdf) {
                ...
                previewTitle.textContent = "Pratinjau Word Baru (DOCX)";
                previewContainer.style.display = 'block';
                docxPreviewDiv.style.display = 'block';
                ...
                previewTitle.textContent = "Pratinjau CSV Baru";
                previewContainer.style.display = 'block';
                docxPreviewDiv.style.display = 'block';
                docxPreviewDiv.innerHTML = '<p class="text-xs text-gray-500 font-bold p-4 text-center">Memuat data CSV...</p>';
