...
<div class="mb-6 page-intro flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
    <div>
        <div class="flex items-center space-x-2 mb-1.5">
            ...
            <span class="text-[11px] font-bold uppercase tracking-widest font-kalam" style="color:var(--muted)">Pengajuan Saya</span>
        </div>
        <h1 class="text-3xl font-bold font-kalam mb-0.5" style="color:var(--txt)">Usulan Perubahan Dokumen</h1>
        <p class="text-sm mt-2" style="color:var(--muted)">Pantau status pengajuan draf revisi dokumen Anda yang sedang ditinjau oleh Administrator.</p>
    </div>
</div>
...
<div id="searchPanel" class="search-panel">
    ...
    <div class="search-bar">
        ...
        <input id="searchInput" type="text" class="search-bar-input" placeholder="Cari judul draf atau dokumen asli…">
    </div>
    <!-- Filter row -->
    <div class="search-filter-row">
        <button class="sf-chip active" data-filter="" data-filter-status="">Semua</button>
        <button class="sf-chip" data-filter="pending" data-filter-status="pending">Pending</button>
        <button class="sf-chip" data-filter="disetujui" data-filter-status="disetujui">Disetujui</button>
        <button class="sf-chip" data-filter="ditolak" data-filter-status="ditolak">Ditolak</button>
        <div class="search-count-badge">
            <span class="scb-num" id="searchCount"><?= count($revisi) ?></span>
            <span>dari <?= count($revisi) ?> usulan</span>
        </div>
    </div>
</div>
...
<div class="flex flex-col gap-5 list-container">
    <?php if (empty($revisi)): ?>
        <div class="glass-card p-14 text-center" style="background-color: #f2e8cf">
            ...
            <p class="text-sm" style="color:var(--muted)">Anda belum pernah mengajukan revisi atau perubahan dokumen.</p>
        </div>
    <?php else: ?>
        <?php 
        ...
        ?>
        <div class="glass-card flex flex-col p-5 anim-item organic-shape" 
             style="background-color: <?= $bgCard ?>"
             data-search="<?= esc(strtolower(($r['judul'] ?? '') . ' ' . ($r['judul_asli'] ?? '') . ' ' . ($r['nama_kategori'] ?? '') . ' ' . ($r['nama_unit'] ?? ''))) ?>"
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-5">
                <div class="flex-1 min-w-0 flex items-start md:items-center gap-5">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-sm font-bold truncate mb-2" style="color:var(--muted)" title="<?= esc($r['judul_asli'] ?? '') ?>">
                            Dokumen Asal: <span style="color:var(--txt)"><?= esc($r['judul_asli'] ?? 'Dokumen Dihapus') ?></span>
                        </p>
                        <div class="flex flex-wrap items-center gap-3 text-[10px] font-bold uppercase tracking-widest font-kalam" style="color:var(--dim)">
                            ...
                            <span class="organic-shape px-2.5 py-1" style="background:rgba(61, 64, 91, 0.08); border:1px solid rgba(61, 64, 91, 0.2)">
                                Unit: <?= esc($r['nama_unit'] ?? '-') ?>
                            </span>
                            <span class="organic-shape px-2.5 py-1" style="background:rgba(61, 64, 91, 0.08); border:1px solid rgba(61, 64, 91, 0.2)">
                                Pengajuan: <?= esc(date('d M Y H:i', strtotime($r['created_at']))) ?>
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <!-- Actions & Status -->
                <div class="flex flex-row md:flex-col items-center md:items-end justify-between md:justify-center shrink-0 gap-3">
            ...
            <?php if ($status === 'ditolak' && !empty($r['pesan_admin'])): ?>
                <div class="mt-4 p-3.5 rounded-xl text-xs font-kalam font-bold" 
                     style="background:rgba(224,122,95,0.08); border:2px dashed rgba(224,122,95,0.4); color:var(--txt);">
                    <div class="text-[10px] text-gray-500 uppercase tracking-wider mb-1">Catatan Penolakan dari Admin:</div>
                    <span style="color:var(--secondary)">"<?= esc($r['pesan_admin']) ?>"</span>
                </div>
            <?php endif; ?>
        </div>
        <?php endforeach; ?>
    ...
    <div class="glass-card w-full max-w-5xl mx-4 relative" style="border-radius:20px;max-height:90vh;overflow:hidden;display:flex;flex-direction:column">
        ...
        <div class="p-6 shrink-0" style="border-bottom:1px solid rgba(139,92,246,.12)">
            <div class="flex items-center justify-between">
                <h3 class="text-sm font-bold grad-violet">Detail Usulan Perubahan</h3>
    ...
    c.innerHTML = `
        <div class="grid grid-cols-2 gap-4 mb-4">
            <div class="p-4 rounded-xl flex flex-col justify-between" style="background:rgba(239,68,68,.06);border:1px solid rgba(239,68,68,.2)">
                <div>
                    <p class="text-[9px] font-bold uppercase tracking-widest mb-2" style="color:rgba(248,113,113,.7)">Dokumen Asal (Versi Asli)</p>
                    <p class="text-sm font-bold mb-1" style="color:var(--txt)">${data.judul_asli ?? '-'}</p>
                    <p class="text-xs mb-3" style="color:var(--muted)">${data.deskripsi_asli ?? 'Tidak ada deskripsi asal.'}</p>
                </div>
                ${data.file_asli ? `
            ...
            <div class="p-4 rounded-xl flex flex-col justify-between" style="background:rgba(16,185,129,.06);border:1px solid rgba(16,185,129,.2)">
                <div>
                    <p class="text-[9px] font-bold uppercase tracking-widest mb-2" style="color:rgba(52,211,153,.7)">Draft Revisi Anda</p>
                    <p class="text-sm font-bold mb-1" style="color:var(--txt)">${data.judul ?? '-'}</p>
                    <p class="text-xs mb-3" style="color:var(--muted)">${data.deskripsi ?? 'Tidak ada deskripsi usulan.'}</p>
                </div>
                ${data.file_dokumen ? `
        ...
        <div class="p-3 rounded-xl text-xs" style="background:rgba(139,92,246,.06);border:1px solid rgba(139,92,246,.15)">
            <p class="text-[9px] font-bold uppercase tracking-widest mb-1" style="color:rgba(196,181,253,.6)">Informasi Pengajuan</p>
            <p style="color:var(--txt)">Diajukan pada: ${data.created_at ?? '-'}</p>
            <p style="color:var(--txt)" class="mt-1">Kategori: ${data.nama_kategori ?? '-'} | Unit: ${data.nama_unit ?? '-'}</p>
        </div>
        <div class="p-4 rounded-xl" style="background:rgba(245,158,11,.08); border:1px solid rgba(245,158,11,.25); margin-top: 20px;">
            <p class="text-[11px] font-bold uppercase tracking-widest mb-1.5" style="color:#D97706">Catatan Perubahan (Pesan Revisi) Anda</p>
        ...
        ${data.pesan_admin ? `
        <div class="p-4 rounded-xl" style="background:rgba(224,122,95,0.08); border:2px dashed rgba(224,122,95,0.4); margin-top: 20px;">
            <p class="text-[11px] font-bold uppercase tracking-widest mb-1.5" style="color:var(--secondary)">Catatan Penolakan dari Admin</p>
            <p style="color:var(--secondary); font-style: italic; font-family:'Lora',serif; font-size:15px; line-height:1.6; white-space:pre-wrap;">"${data.pesan_admin}"</p>
        </div>
        ` : ''}
    `;
